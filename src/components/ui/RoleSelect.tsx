  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <svg
          className={cn('w-4 h-4 text-slate-400 light:text-slate-500 shrink-0 transition-transform', open && 'rotate-180')}
      ...
      {open && (
        <div className="absolute z-50 top-full mt-1 w-full bg-slate-800 light:bg-white border border-slate-600 light:border-slate-300 rounded-lg shadow-xl overflow-hidden max-h-64 overflow-y-auto">
          {ROLES.map((role) => {
            ...
            return (
              <button
                ...
                onClick={() => toggle(role)}
                className={cn(
                  'w-full flex items-center gap-3 px-3 py-2.5 text-sm text-left hover:bg-slate-700 light:hover:bg-slate-100 transition',
                  selected ? 'text-white light:text-slate-900' : 'text-slate-300 light:text-slate-700'
                )}
              >
                <span
                  className={cn(
                    ...
                    selected
                      ...
                      : 'border-slate-500 light:border-slate-300'
                  )}
                >
                  {selected && (
