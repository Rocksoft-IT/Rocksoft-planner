...
import { calcUtilization, formatAvailability, getAllocationStyle, hexToRgba, cn } from '@/lib/utils'
          ...
          {rowData.map(({ person, laned, personAllocs, personOffs, rowHeight }) => {
            ...
            <div key={person.id} className="flex border-b border-slate-800" style={{ height: rowHeight }}>
              ...
              {laned.map((item) => {
                ...
                // Keep the project name visible: if the block starts before the
                // viewport's left edge, slide the label right to the visible edge,
                // clamped so it never runs past the block's right edge (869e6rn52).
